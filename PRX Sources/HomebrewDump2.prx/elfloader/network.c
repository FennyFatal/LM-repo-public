...
bool network_init(void) {
	...
	ret = sceNetInit();
	if (ret < 0) {
		klog("sceNetInit failed: %d\n", sce_net_errno);
		goto err;
	}

	ret = sceNetPoolCreate("payload_ldr", NET_HEAP_SIZE, 0);
	if (ret < 0) {
		klog("sceNetPoolCreate failed: %d\n", sce_net_errno);
		goto err_net_term;
	}
	s_libnet_mem_id = ret;
...
err_pool_destroy:
	if (s_libnet_mem_id >= 0) {
		...
		if (ret < 0) {
			klog("sceNetPoolDestroy failed: %d\n", sce_net_errno);
		}
