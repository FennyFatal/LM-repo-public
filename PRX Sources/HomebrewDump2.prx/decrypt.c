...
//#include "C:\Users\sethk\Desktop\ps4-payload-sdk-master\libPS4\include\ps4.h"
...
void print_phdr(Elf64_Phdr *phdr) {
	//printfkernel("=================================\n");
	//printfkernel("     p_type %08x\n", phdr->p_type);
	//printfkernel("     p_flags %08x\n", phdr->p_flags);
	//printfkernel("     p_offset %016llx\n", phdr->p_offset);
	//printfkernel("     p_vaddr %016llx\n", phdr->p_vaddr);
	//printfkernel("     p_paddr %016llx\n", phdr->p_paddr);
	//printfkernel("     p_filesz %016llx\n", phdr->p_filesz);
	//printfkernel("     p_memsz %016llx\n", phdr->p_memsz);
	//printfkernel("     p_align %016llx\n", phdr->p_align);
}
...
int is_segment_in_other_segment(Elf64_Phdr *phdr, int index, Elf64_Phdr *phdrs, int num) {
	for (int i = 0; i < num; i += 1) {
		...
		if (i != index) {
			if (p->p_filesz > 0) {
				// //printfkernel("offset : %016x,  toffset : %016x\n", phdr->p_offset, p->p_offset);
				// //printfkernel("offset : %016x,  toffset + size : %016x\n", phdr->p_offset, p->p_offset + p->p_filesz);
	...
	for (int i = 0; i < num; i += 1) {
		...
		if (phdr->p_filesz > 0 && phdr->p_type != 0x6fffff01) {
			if (!is_segment_in_other_segment(phdr, i, phdrs, num)) {
				...
				// //printfkernel("seg buf info %d -->\n", segindex);
				// //printfkernel("    index : %d\n    bufsz : 0x%016llX\n", info->index, info->bufsz);
				// //printfkernel("    filesz : 0x%016llX\n    fileoff : 0x%016llX\n", info->filesz, info->fileoff);
			}
		}
	}
	*segBufNum = segindex;
