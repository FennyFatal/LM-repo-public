...
public class GamePad : MonoBehaviour
{
	...
	void DPadButtons()
	{
        ...
        if (Input.GetAxisRaw("Right-Dpad") == 0) //right dpad
        ...
        if (Input.GetAxisRaw("UP-Dpad") == 0) //down dpad
            gamePad.dpad_up.color = inputOn;
		else
			gamePad.dpad_up.color = inputOff;

        //if(Input.GetAxis("dpad" + stickID + "_vertical") < 0)
       if (Input.GetAxisRaw("UP-Dpad") == -1) //up dpad
	...
	void TriggerShoulderButtons()
	{
		...
		if(Input.GetAxis("_left_trigger") != 0)
			gamePad.L2.color = inputOff+(inputOn*Input.GetAxis("_left_trigger"));
		else
			gamePad.L2.color = inputOff;
		
		if(Input.GetAxis("_right_trigger") != 0)
			gamePad.R2.color = inputOff+(inputOn*-Input.GetAxis("_right_trigger"));
		else
